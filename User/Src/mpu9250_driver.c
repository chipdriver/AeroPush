...
{
    // 加速度转换：单位 g
    physical->accel_x_g = (float)raw->accel_x / 4096.0f; // ±8g 对应 4096 LSB/g
    physical->accel_y_g = (float)raw->accel_y / 4096.0f;
    physical->accel_z_g = (float)raw->accel_z / 4096.0f;

    // 陀螺仪转换：单位 °/s
    physical->gyro_x_dps = (float)raw->gyro_x / 32.8f; // ±1000°/s 对应 32.8 LSB/(°/s)
    physical->gyro_y_dps = (float)raw->gyro_y / 32.8f;
    physical->gyro_z_dps = (float)raw->gyro_z / 32.8f;
